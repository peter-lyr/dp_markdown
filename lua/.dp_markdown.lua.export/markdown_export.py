...
def get_html(toc, html):
  ...
  if 'div class="mermaid"' in html:
    js_mermaid = (
      ' <script type="text/javascript" src="https://unpkg.com/mermaid@8.11.0/dist/mermaid.js" crossorigin="anonymous"></script>\n'
...
def put_html_to_file(html, f):
  html = html.replace(
    "</body>",
    '<script type="text/javascript">window.onload=function(){window.status="ready";}</script>\n\n </body>',
  )
  with open(f, "wb") as f:
...
def render_html_file(html_p):
  ...
  driver = webdriver.Chrome(
    ChromeDriverManager(version="87.0.4280.88").install(), options=options
  )
...
def image2base64(html_p):
  ...
  for img_src in img_srcs:
    ...
    try:
      with open(
        os.path.normpath(os.path.join(os.path.dirname(html_p), img_src)), "rb"
      ) as f:
        image_bytes = base64.b64encode(f.read())
    except Exception as e:
      print("""os.path.curdir: """, os.path.curdir)
      print(
        """os.path.abspath(os.path.curdir): """, os.path.abspath(os.path.curdir)
      )
      print("""err: """, e)
      continue
